...
/*
	...
	Examples
		XMLToArray -- Convert XML to Associative Array
			Usage:		$xml = new XMLToArray( xmldata (string), 
                                                ignorefields (array 1,2,3), 
                                                replacefields(array OLD => NEW), 
                                                show attributes?, 
                                                convert to upper );

						$array = $xml->getArray();

			Example:	$xml = new XMLToArray( 'http://www.slashdot.org/slashdot.xml', 
                                                array( 'backslash' ), 
                                                array( 'story' => '_array_' ), 
                                                true, false );
		...
		ArrayToXML -- Convert Associative Array to XML
			Usage:		$array = new ArrayToXML( $xml->getArray(), 
                                                $xml->getReplaced(), 
                                                $xml->getAttributes() )
						$xml = $array->getXML();
*/
...
class XMLToArray {
	...
	function XMLToArray( $data, $ignore = Array(), $replace = Array(), 
                                        $showattribs = false, $toupper = false) 
        {
	...
	function & getAmpArray() {
		return $this->_data[0]['track'];
	}
	...
	function _endElement( $parser, $name ) {
		if ( ! in_array( $name, $this->_ignore ) && 
                                isset( $this->_name[$this->_level - 1] ) ) 
                {
	...
	function _addElement( &$name, &$start, $add = array(), $isattribs = false ) {
		...
		if ( ! empty( $this->_replace[$name] ) && 
                                ( '_ARRAY_' === strtoupper( $this->_replace[$name] ) ) ) 
                {
			if ( ! $start[$name] )
				$this->_rep[$name] = 0;
